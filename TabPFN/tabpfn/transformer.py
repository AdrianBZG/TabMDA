...
class TransformerModel(nn.Module):
    ...
    def encode(self, x_src, y_src, style, eval_pos, src_mask=None):
        x_src = self.encoder(x_src)
        y_src = self.y_encoder(y_src.unsqueeze(-1) if len(y_src.shape) < len(x_src.shape) else y_src)
        style_src = self.style_encoder(style).unsqueeze(0) if self.style_encoder else \
        ...
        if src_mask is None:
            if self.global_att_embeddings is None:
                ...
                if self.full_attention:
                    ...
                    src_mask = eval_pos + len(style_src)
                else:
                    src_mask = self.generate_D_q_matrix(full_len, len(x_src) - eval_pos).to(x_src.device)
            else:
                src_mask_args = (self.global_att_embeddings.num_embeddings,
                                 len(x_src) + len(style_src),
                                 len(x_src) + len(style_src) - eval_pos)
        ...
        src = torch.cat([global_src, style_src, train_x, x_src[eval_pos:]], 0)
        ...
        x_src_enc = self.transformer_encoder(src, src_mask)
        return x_src_enc

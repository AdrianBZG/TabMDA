...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _validate_targets(self, y):
        if type(y) == torch.Tensor:
            # This allows y to be a torch.Tensor on the GPU
            y = y.cpu().numpy()
...
class TabPFNEncoder(TabPFNClassifier):
    def __init__(self, **kwargs):
        super(TabPFNEncoder, self).__init__(**kwargs)

    def encode(self, X_to_encode, X_context, y_context, model):
        # Prepare the X to be encoded (e.g. one sample/row) together with the context (e.g. N samples/rows)
        X_full = torch.cat([X_context, X_to_encode], dim=0).float().unsqueeze(1)

        # Store the classes seen during fit
        #y_context = self._validate_targets(y_context)
        #y_context = LabelEncoder().fit_transform(y_context)
        y_full = y_context.float().unsqueeze(1).to(self.device)
        ...
        with warnings.catch_warnings():
            ...
            X_encoded = transformer_encoder(model, X_full, y_full, eval_pos,
                                            device=self.device,
...
def transformer_encoder(model, eval_xs, eval_ys, eval_position,
                        device='cpu',
                        max_features=100,
                        inference_mode=False,
                        extend_features=True,
    ...
    styles = style_.repeat(eval_xs.shape[1], 1) if style_ is not None else None
    ...
    for batch_input, batch_label in zip(inputs, labels):
        if device == 'cpu':
            output_batch = model.encode(x_src=batch_input, y_src=batch_label, style=styles, eval_pos=eval_position)
        else:
            with torch.cuda.amp.autocast(enabled=fp16_inference):
                output_batch = model.encode(x_src=batch_input, y_src=batch_label, style=styles, eval_pos=eval_position)
    ...
    outputs = torch.cat(outputs, 1)[eval_position:]
    return outputs
